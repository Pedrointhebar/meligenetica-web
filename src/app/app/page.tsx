    ...
    <div style={{ width: size, height: size, position: 'relative', flexShrink: 0 }}>
      <svg width={size} height={size} style={{ transform: 'rotate(-90deg)', position: 'absolute' }}>
        ...
        <circle cx={size/2} cy={size/2} r={r} fill="none" stroke={col} strokeWidth={4}
          ...
          strokeDashoffset={offset} className="score-ring-fill"/>
      </svg>
      <div style={{
        position: 'absolute', inset: 0, display: 'flex',
        flexDirection: 'column', alignItems: 'center', justifyContent: 'center',
      }}>
        <span style={{ fontSize: size * 0.24, fontWeight: 800, color: col, letterSpacing: '-0.5px', lineHeight: 1 }}>
          {score}
        </span>
        <span style={{ fontSize: size * 0.11, fontWeight: 600, color: col, marginTop: 1 }}>
          {scoreLabel(score)}
        </span>
      </div>
    </div>
  ...
  return (
    <div style={{ height: 5, background: C.border, borderRadius: 99, overflow: 'hidden', marginTop: 6 }}>
      <div className="prog-fill"
        style={{ height: '100%', width: `${(value / max) * 100}%`, background: color, borderRadius: 99 }}/>
    </div>
  )
}
...
function Chip({ label }: { label: string }) {
  return (
    <span style={{
      ...
      background: C.bg, borderRadius: 5, padding: '2px 7px',
    }}>
      {label}
    </span>
  )
...
function SliderField({
  label, name, value, onChange, min, max, hint, color,
}: {
  label: string; name: string; value: number;
  onChange: (v: number) => void; min: number; max: number; hint?: string; color?: string
}) {
  return (
...
function ModalCheckin({
  colmeia, onSave, onClose
}: { colmeia: Colmeia; onSave: (c: CheckIn) => void; onClose: () => void }) {
  ...
  const delta = ult
    ? (novoScore - calcScore({ producao: colmeia.producaoAnual, ...ult })).toFixed(1)
    : null
  ...
  return (
    <Modal title="Check-in Semanal" sub={colmeia.nome} onClose={onClose}>
      ...
      <div style={{ display: 'flex', gap: 20, padding: '12px 20px 16px', alignItems: 'center' }}>
        ...
        <div>
          ...
          <div style={{ fontSize: 16, fontWeight: 700, marginTop: 2 }}>
            🍯 {colmeia.producaoAnual} kg / {colmeia.anoProducao}
          </div>
          {delta !== null && (
            <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginTop: 8 }}>
              ...
              <span style={{
                ...
                color: parseFloat(delta) > 0 ? C.green : parseFloat(delta) < 0 ? C.red : C.text3,
              }}>
      ...
      <div style={{ background: C.bg, borderRadius: 12, margin: '0 16px 16px' }}>
        <SliderField label="Força populacional" name="pop" value={pop} onChange={setPop} min={0} max={10}
          hint="Frames cobertos (0–10)" color={C.green}/>
        <SliderField label="Mansidão" name="man" value={man} onChange={setMan} min={0} max={5}
          hint="0 = defensiva · 5 = completamente mansa" color="#007AFF"/>
        <SliderField label="Sanidade geral" name="san" value={san} onChange={setSan} min={0} max={5}
          hint="0 = parasitada/doente · 5 = saudável" color={C.red}/>
        <SliderField label="Atividade de voo" name="atv" value={atv} onChange={setAtv} min={0} max={5}
          hint="Forrageamento observado" color={C.orange}/>
      </div>
      ...
      <button onClick={() => onSave({
        id: crypto.randomUUID(), data, populacao: pop,
        mansidao: man, sanidade: san, atividade: atv, notas,
      })}
        style={{
          display: 'block', width: 'calc(100% - 32px)', margin: '0 16px',
          background: C.amber, color: 'white', border: 'none', borderRadius: 12,
...
function ModalColheita({
  colmeia, onSave, onClose
}: { colmeia: Colmeia; onSave: (kg: number, ano: number) => void; onClose: () => void }) {
  ...
  return (
    <Modal title="Registro de Colheita" sub={colmeia.nome} onClose={onClose}>
      <div style={{ background: C.bg, borderRadius: 12, margin: '16px 16px 16px' }}>
        <FormField label="Total colhido (kg)">
          <input type="number" min={0} step={0.1} value={kg}
            onChange={e => setKg(parseFloat(e.target.value) || 0)}
            style={{ border: 'none', background: 'transparent', fontSize: 17, fontWeight: 700,
              color: C.amber, width: 80, textAlign: 'right', outline: 'none' }}/>
        </FormField>
      ...
      <div style={{ margin: '0 16px 20px' }}>
        <div style={{ fontSize: 12, fontWeight: 700, color: C.text3, textTransform: 'uppercase', letterSpacing: 0.8, marginBottom: 8 }}>
          Referência — Meliponini
        </div>
        <Card>
...
function ModalNovaColmeia({ onSave, onClose }: {
  onSave: (c: Colmeia) => void; onClose: () => void
}) {
  ...
  return (
    <Modal title="Nova Colmeia" onClose={onClose}>
      <div style={{ background: C.bg, borderRadius: 12, margin: '16px 16px 12px' }}>
        <FormField label="Nome / ID">
          <input value={nome} onChange={e => setNome(e.target.value)}
            placeholder="ex: Rainha Alfa"
            style={{ border: 'none', background: 'transparent', fontSize: 15,
              color: C.text, textAlign: 'right', outline: 'none', flex: 1 }}/>
        </FormField>
        ...
        <FormField label="Produção anual (kg)">
          <input type="number" min={0} step={0.1} value={prod}
            onChange={e => setProd(parseFloat(e.target.value)||0)}
            style={{ border: 'none', background: 'transparent', fontSize: 15,
              color: C.amber, textAlign: 'right', width: 70, outline: 'none' }}/>
        </FormField>
      ...
      <div style={{ background: C.bg, borderRadius: 12, margin: '0 16px 16px' }}>
        <SliderField label="Força populacional" name="pop" value={pop} onChange={setPop} min={0} max={10} color={C.green}/>
        <SliderField label="Mansidão" name="man" value={man} onChange={setMan} min={0} max={5} color="#007AFF"/>
        <SliderField label="Sanidade geral" name="san" value={san} onChange={setSan} min={0} max={5} color={C.red}/>
        <SliderField label="Atividade de voo" name="atv" value={atv} onChange={setAtv} min={0} max={5} color={C.orange}/>
      </div>
      <button
        disabled={!nome.trim()}
        onClick={() => onSave({
          ...
          historico: [{
            ...
            populacao: pop, mansidao: man, sanidade: san, atividade: atv, notas: '',
          }],
        })}
        style={{
          display: 'block', width: 'calc(100% - 32px)', margin: '0 16px',
          background: nome.trim() ? C.amber : C.border,
          color: nome.trim() ? 'white' : C.text3,
          border: 'none', borderRadius: 12, padding: '15px',
          fontSize: 16, fontWeight: 700, cursor: nome.trim() ? 'pointer' : 'default',
        }}>
        Adicionar Colmeia
      </button>
    </Modal>
...
function DetalheView({
  c, onBack, onCheckin, onColheita, onDelete
}: {
  c: Colmeia; onBack: () => void;
  onCheckin: () => void; onColheita: () => void; onDelete: () => void
}) {
  const [tab, setTab] = useState<'overview'|'historico'>('overview')
  const s = currentScore(c)
  const snap = lastSnap(c)
  const t = trend(c)
  const div = canSplit(c)
    ...
    <div>
      ...
      <div style={{
        ...
        <button onClick={onBack}
          style={{ border: 'none', background: 'none', cursor: 'pointer',
            color: C.amber, fontSize: 15, fontWeight: 600, display: 'flex', alignItems: 'center', gap: 4 }}>
          ‹ Colmeias
        </button>
      ...
      <div style={{ padding: 20 }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 16 }}>
            <div style={{ flex: 1 }}>
              ...
              <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginTop: 10 }}>
                ...
                {div && (
                  <span style={{ fontSize: 11, fontWeight: 700, color: C.green,
                    background: `${C.green}18`, borderRadius: 99, padding: '2px 8px' }}>
                    ✓ Apta para divisão
                  </span>
                )}
              </div>
              <div style={{
                ...
                background: `${C.amber}12`, borderRadius: 10,
                display: 'flex', justifyContent: 'space-between', alignItems: 'center',
              }}>
                <div>
                  <div style={{ fontSize: 11, color: C.text3 }}>Produção anual ({c.anoProducao})</div>
                  <div style={{ fontSize: 22, fontWeight: 800, color: C.amber, marginTop: 2 }}>
                    🍯 {c.producaoAnual} kg
                  </div>
                </div>
                <button onClick={onColheita}
                  style={{ border: 'none', background: `${C.amber}18`, borderRadius: 8,
                    padding: '6px 12px', color: C.amber, fontWeight: 700, cursor: 'pointer', fontSize: 13 }}>
                  Atualizar
                </button>
              </div>
            </div>
        ...
        <div style={{ display: 'flex', gap: 10, marginTop: 12 }}>
          <button onClick={onCheckin} style={{
            flex: 1, background: C.amber, color: 'white', border: 'none',
        ...
        {/* Seg control */}
        <div style={{
          display: 'flex', background: '#e8e0d4', borderRadius: 10,
          ...
          {(['overview','historico'] as const).map(tt => (
            <button key={tt} onClick={() => setTab(tt)}
              style={{
                ...
                background: tab === tt ? 'white' : 'transparent',
                color: tab === tt ? C.text : C.text3,
                boxShadow: tab === tt ? '0 1px 4px rgba(0,0,0,0.12)' : 'none',
              }}>
      ...
      {tab === 'overview' && (
        <div style={{ padding: '0 20px 32px' }}>
              ...
              <Card style={{ padding: '14px 8px 8px' }}>
                <ResponsiveContainer width="100%" height={160}>
                  <AreaChart data={chartData} margin={{ top:4, right:12, bottom:0, left:-16 }}>
                    ...
                    <Area type="monotone" dataKey="score" stroke={C.amber} strokeWidth={2.5}
                      fill="url(#ambar)" dot={{ fill:C.amber, r:3 }} name="Score"/>
                  </AreaChart>
                </ResponsiveContainer>
              </Card>
            </>
          ...
          <div style={{
            padding: 14, background: `${C.amber}10`, borderRadius: 12,
            border: `1px solid ${C.amber}30`, fontSize: 14, lineHeight: 1.6, color: C.text2,
          }}>
            🧬 {s >= 70
...
export default function AppPage() {
  ...
  const scoreMedia = useMemo(() => {
    if (!colmeias.length) return 0
    return Math.round(colmeias.reduce((a, c) => a + currentScore(c), 0) / colmeias.length * 10) / 10
  }, [colmeias])
  ...
  const pendentes = colmeias.filter(c => {
    const u = lastSnap(c)
    return !u || new Date(u.data) < lim
  })

  // Histórico meliponário
  ...
  const meliHist = maxSem >= 2 ? Array.from({ length: Math.min(8, maxSem) }, (_, i) => {
    ...
    const vals = colmeias.flatMap(c => {
      const idx = c.historico.length - n + i
      if (idx < 0) return []
      return [calcScore({ producao: c.producaoAnual, ...c.historico[idx] })]
    })
    const ref = colmeias.find(c => c.historico.length >= n - i)
    const h = ref?.historico[ref.historico.length - n + i]
    const label = h ? new Date(h.data).toLocaleDateString('pt-BR',{day:'2-digit',month:'2-digit'}) : `S${i+1}`
    return { sem: label, score: vals.length ? Math.round(vals.reduce((a,b)=>a+b,0)/vals.length*10)/10 : 0 }
  }) : []
    ...
    <div style={{ background: C.bg, minHeight: '100vh', maxWidth: 520, margin: '0 auto', position: 'relative' }}>
          ...
          <>
            ...
            {tab === 'painel' && (
              <div style={{ padding: 16 }}>
                <h1 style={{ fontSize: 32, fontWeight: 800, letterSpacing: '-0.5px', marginBottom: 16,
                ...
                {pendentes.length > 0 && (
                  <div style={{
                    ...
                    <button onClick={() => setModalCheckin(pendentes[0])}
                      style={{ background: C.orange, color: 'white', border: 'none', borderRadius: 8,
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 18 }}>
                    ...
                    <div>
                      ...
                      <div style={{ fontSize: 14, color: C.text2, marginTop: 4 }}>
                        {colmeias.length} colmeia{colmeias.length !== 1 ? 's' : ''}
                      </div>
                      <div style={{ fontSize: 14, color: C.green, marginTop: 2 }}>
                        ✓ {colmeias.filter(canSplit).length} apta{colmeias.filter(canSplit).length !== 1 ? 's' : ''} p/ divisão
                      </div>
                      {ranking[0] && (
                        <div style={{ marginTop: 10, fontSize: 14 }}>
                          🏆 <strong>{ranking[0].nome}</strong>
                          <div style={{ fontSize: 11, color: C.text3, fontStyle: 'italic' }}>
                            {ESPECIES.find(e => e.id === ranking[0].especie)?.sci}
                          </div>
                        </div>
                      )}
                    </div>
                ...
                {/* Stats */}
                <SectionHdr title="Resumo"/>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                  {[
                    { icon:'🍯', label:'Produção total', value:`${colmeias.reduce((a,c)=>a+c.producaoAnual,0).toFixed(1)} kg`, sub:'soma anual' },
                    { icon:'🪲', label:'Aptas à divisão', value:colmeias.filter(canSplit).length, sub:`de ${colmeias.length}`, color: colmeias.filter(canSplit).length > 0 ? C.green : undefined },
                  ].map(s => (
                    <Card key={s.label} style={{ padding: '14px 16px' }}>
                      <div style={{ fontSize: 24, marginBottom: 6 }}>{s.icon}</div>
                      <div style={{ fontSize: 26, fontWeight: 800, color: s.color || C.text, letterSpacing: '-0.5px' }}>
                        {s.value}
                      </div>
                ...
                {meliHist.length >= 2 && (
                  <>
                    <SectionHdr title="Tendência do plantel"/>
                    <Card style={{ padding: '14px 8px 8px' }}>
                      <ResponsiveContainer width="100%" height={140}>
                        <AreaChart data={meliHist} margin={{ top:4, right:12, bottom:0, left:-16 }}>
                          <defs>
                          ...
                          <YAxis domain={[0,100]} tick={{ fontSize:10, fill:C.text3 }} axisLine={false} tickLine={false}/>
                          <Tooltip contentStyle={{ fontSize:12, borderRadius:8, border:'none', boxShadow:'0 4px 12px rgba(0,0,0,0.12)' }}/>
                          <Area type="monotone" dataKey="score" stroke={C.amber} strokeWidth={2.5} fill="url(#ag)" name="Score"/>
                        </AreaChart>
                      </ResponsiveContainer>
                    </Card>
                  </>
                ...
                {/* Distribuição */}
                <SectionHdr title="Distribuição fenotípica"/>
                <Card>
                  {ranking.map((c, i, arr) => {
                    const s = currentScore(c)
                    return (
                      <div key={c.id} style={{
                        ...
                        <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13, marginBottom: 4 }}>
                          ...
                          <span style={{ fontWeight: 800, color: scoreColor(s) }}>{s}</span>
                        </div>
                        <ProgBar value={s} max={100} color={scoreColor(s)}/>
                      </div>
                    )
                  })}
                </Card>
            ...
            {tab === 'ranking' && (
              <div style={{ padding: 16 }}>
                <h1 style={{ fontSize: 32, fontWeight: 800, letterSpacing: '-0.5px', marginBottom: 16,
                  fontFamily: 'Georgia, serif' }}>Ranking</h1>
                <div style={{ fontSize: 12, color: C.text3, marginBottom: 12,
                  background: `${C.amber}10`, borderRadius: 10, padding: '8px 12px' }}>
                      ...
                      <div key={c.id} className="card-hover"
                        ...
                        <div style={{ width: 28, textAlign: 'center', flexShrink: 0 }}>
                          {medal
                            ...
                            : <span style={{ fontWeight: 700, color: C.text3 }}>{i+1}</span>}
                        </div>
                        <div style={{ flex: 1, minWidth: 0 }}>
                          <div style={{ fontWeight: 600, fontSize: 16 }}>{c.nome}</div>
                          <div style={{ fontSize: 12, color: C.text3, fontStyle: 'italic', marginTop: 1 }}>
                            {ESPECIES.find(e => e.id === c.especie)?.sci}
                          </div>
                          <div style={{ display: 'flex', gap: 5, flexWrap: 'wrap', marginTop: 6 }}>
                            {div && <span style={{ fontSize: 11, fontWeight: 700, color: C.green,
                              background: `${C.green}18`, borderRadius: 99, padding: '2px 8px' }}>✓ Dividir</span>}
                            <TrendBadge t={t}/>
                            <span style={{ fontSize: 11, color: C.text3, background: C.bg, borderRadius: 99, padding: '2px 8px' }}>
                              🍯 {c.producaoAnual}kg
                            </span>
                          </div>
                        </div>
              ...
              <div style={{ padding: 16 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
                  <h1 style={{ fontSize: 32, fontWeight: 800, letterSpacing: '-0.5px',
                    fontFamily: 'Georgia, serif' }}>Colmeias</h1>
                  <button onClick={() => setModalNova(true)}
                    style={{ background: C.amber, color: 'white', border: 'none',
                      borderRadius: 10, padding: '8px 18px', fontWeight: 700, fontSize: 15, cursor: 'pointer' }}>
                    + Nova
                  </button>
                </div>
                {colmeias.length === 0
                        ...
                        return (
                          <div key={c.id} className="card-hover"
                            ...
                            <div style={{
                              ...
                              background: `${scoreColor(s)}18`, display: 'flex',
      ...
      {!detalhe && (
        <div style={{
          ...
          background: 'rgba(247,243,238,0.95)', backdropFilter: 'blur(12px)',
          ...
          {([
            ...
            <button key={t.id} onClick={() => setTab(t.id)} style={{
              ...
              <span style={{ fontSize: tab === t.id ? 22 : 20, filter: tab === t.id ? 'none' : 'grayscale(1) opacity(0.5)' }}>
                {t.icon}
              </span>
              <span style={{ fontSize: 10, fontWeight: 600, color: tab === t.id ? C.amber : C.text3 }}>
                {t.label}
              </span>
            </button>
          ))}
